...
export async function retrieveSession(sessionId: string, options?: { expand?: string[] }) {
...
export async function updateProduct(productId: string, product: Stripe.ProductUpdateParams) {
    await stripe.products.update(productId, product);
}

type CheckoutSessionParams<T = unknown> = Stripe.Checkout.SessionCreateParams & T;
