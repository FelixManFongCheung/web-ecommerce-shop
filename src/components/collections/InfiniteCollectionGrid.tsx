...
export default function InfiniteCollectionGrid({
  ...
  const updateColumns = () => {
    if (window.matchMedia(`(min-width: ${MediaQuerySizes.LG}px)`).matches) {
      ...
      window.matchMedia(`(min-width: ${MediaQuerySizes.MD}px)`).matches
    ) {
      setColumns(3);
    } else if (
      window.matchMedia(`(min-width: ${MediaQuerySizes.SM}px)`).matches
    ) {
      setColumns(2);
    } else {
      setColumns(2);
  ...
  return (
    <>
                  ...
                  return (
                    ...
                    >
                      <Suspense fallback={<ProductCardSkeleton />}>
                        <ClientProductCard product={product} className="mb-6">
                          <Link
      ...
      {isFetchingNextPage && <GridProductSkeletonCards />}
